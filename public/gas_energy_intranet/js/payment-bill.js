			...
			success: function(paymentPlanData) {
				...
				if (paymentPlanData.models[0].attributes.details.planAmountDue &&
					...
					$('.totalMonthlyPaymentPlanSummaryAmount').html('$' +
						paymentPlanData.models[0].attributes.details.planAmountDue.amount);
...
var CurrentBillData = Backbone.Model.extend({
	url: host+'/currentBillDataPayment',
	defaults: {
		currentBillData: "",
		status: false,
		accountIndex: 0,
...
var billCmpModel = Backbone.Model.extend({
	...
	url: function() {
		...
		return host+'/billComparison0101';
	},
	defaults: {
		currentBillData: "",
		accountIndex: 0
...
var invoiceLineItemsModel = Backbone.Model.extend({
	...
	url: function() {
		...
		return host+'/invoiceLineItemsPayment';
	}
});
